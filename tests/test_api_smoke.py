...
def test_watchlist_shape() -> None:
    ...
    assert "latest_decision_batch_cue" in payload["items"][0]["latest_context"]
    assert "latest_decision_rationale_cue" in payload["items"][0]["latest_context"]
...
def test_orchestration_review_shape() -> None:
    ...
    if payload["plans"]:
        ...
        assert "compact_rationale_cue" in payload["plans"][0]
